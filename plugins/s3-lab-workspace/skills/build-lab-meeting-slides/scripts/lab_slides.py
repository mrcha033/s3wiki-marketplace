#!/usr/bin/env python3
"""Project, evidence, build, and review helper for lab-meeting decks."""
...
def derive_template_profile(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "authoring_mode": "duplicate mapped source slides, edit inherited targets, and add only inside the inspected content zones",
    }
...
def run_template_inspection(config: dict[str, Any], config_path: Path, project: Path) -> dict[str, Any]:
    ...
    if retained_lab_template:
        ...
        default_visual_motif = "locked S3/Yonsei chrome with an editable technical mechanism inside the inherited body zone"
    else:
        ...
        default_visual_motif = "locked inherited template chrome with editable research content inside the approved content zone"
    ...
    design.update(
        {
            ...
            "visual_motif": design.get("visual_motif") if preserve_design_declaration and design.get("visual_motif") else default_visual_motif,
            "anti_patterns": [
    ...
    audit_lines = [
        ...
        "Authoring: duplicate source slides, edit inherited targets, and add only inside inspected content zones.",
    ]
...
def declared_advanced_edit_actions(config: dict[str, Any], project: Path) -> set[str]:
    advanced_edit = config.get("build", {}).get("advanced_edit")
    if advanced_edit in (None, False):
        return set()
    if not isinstance(advanced_edit, dict) or advanced_edit.get("enabled") is not True:
        raise LabDeckError("build.advanced_edit must be an object with enabled=true")
    edit_id = str(advanced_edit.get("id", "")).strip()
    rationale = str(advanced_edit.get("rationale", "")).strip()
    raw_actions = advanced_edit.get("actions")
    if not edit_id or not rationale or not isinstance(raw_actions, list) or not raw_actions:
        raise LabDeckError("advanced edits require nonempty id, rationale, and actions")
    actions = {str(action).strip() for action in raw_actions if str(action).strip()}
    if len(actions) != len(raw_actions):
        raise LabDeckError("advanced edit actions must be unique nonempty strings")
    builder = resolve_path(config.get("build", {}).get("builder"), project)
    if not builder or not builder.exists():
        raise LabDeckError("advanced edits require an existing project builder")
    marker = f"LABDECK_ADVANCED_EDIT: {edit_id}"
    if marker not in builder.read_text(encoding="utf-8"):
        raise LabDeckError(f"advanced edit is not declared by the builder; add marker {marker!r}")
    return actions
    ...
    for index, target in enumerate(targets):
        ...
        if action not in {"rewrite", "add"}:
            raise LabDeckError(
                ...
                "declare a marked build.advanced_edit for deck-specific handling"
            )
        if action == "rewrite":
...
def command_init(args: argparse.Namespace) -> int:
    ...
    config = {
        ...
        "deck": {
            ...
            "communication_job": "",
            "duration_minutes": 30,
            "authoring_mode": "template-native",
...
def audit_config(config: dict[str, Any], config_path: Path, project: Path) -> dict[str, Any]:
    ...
    if config.get("schema_version") == 2 and template_mode(config):
        mode = str(deck.get("authoring_mode", "")).strip()
        if mode != "template-native":
            errors.append(f"unsupported template authoring mode: {mode!r}")
    ...
    if config.get("schema_version") == 2:
        if not design_path.exists():
            ...
            for field in ("visual_thesis", "system_declaration", "visual_motif"):
    ...
    errors.extend(validate_notes(config, project, len(slides), inherited_notes_deck))
    ...
    if template_mode(config):
        try:
            ...
            errors.append(f"template project could not be checked: {error}")
...
def command_finalize(args: argparse.Namespace) -> int:
    ...
    note_errors = validate_notes(config, project, len(plan_data["slides"]), input_deck)
    if note_errors:
        raise LabDeckError("speaker-note check failed: " + "; ".join(note_errors))
...
def command_run(args: argparse.Namespace) -> int:
    ...
    return command_qa(argparse.Namespace(config=args.config))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    inspect_template = subparsers.add_parser(
        "inspect-template",
        help="inspect every template slide and derive its reusable layout profile",
    )
    ...
    qa = subparsers.add_parser("qa", help="check package, renders, notes, compatibility, and review")
