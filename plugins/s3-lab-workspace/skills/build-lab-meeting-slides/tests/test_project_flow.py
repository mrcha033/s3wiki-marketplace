...
SPEC = importlib.util.spec_from_file_location("lab_slides_project_under_test", MODULE_PATH)
...
def create_project(root: Path, builder_text: str) -> tuple[dict, Path, Path]:
    project = root / "project"
    (project / "build").mkdir(parents=True)
    (project / "content").mkdir()
    (project / "work/template/template-inspect").mkdir(parents=True)
    (project / "build/builder.mjs").write_text(builder_text, encoding="utf-8")
        ...
        "slides": [
            {
                ...
                "title_claim": "The inherited frame keeps the lab identity visible",
                "template_frame": {
                    "role": "cover",
                    "source_slide": 1,
                    "routing_rationale": "The first supplied slide is the intended cover.",
                },
                "visual_anchor": "Inherited cover typography and lockup",
                "coverage": ["opening"],
                "evidence_refs": [],
                "notes_ref": "slide-1",
            }
    ...
    write_json(
        project / "content/design-system.json",
        {
            ...
            "system_declaration": "Reuse the supplied source frames and edit mapped targets.",
            "visual_motif": "Inherited lab chrome with editable research content.",
        },
    )
    config = {
        "schema_version": 2,
        "deck": {
            ...
            "communication_job": "The lab should understand why template inheritance protects clarity.",
...
class ProjectFlowTests(unittest.TestCase):
    def test_load_config_accepts_supported_versions(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "labdeck.json"
    ...
    def test_init_creates_a_small_template_native_project(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            project = Path(temporary) / "new-deck"
            ...
            self.assertNotIn("require_review_reports", config["qa"])
            self.assertFalse((project / "reports/pass-a.md").exists())
            self.assertFalse((project / "reports/pass-b.md").exists())

    def test_builder_reuses_the_prepared_starter(self) -> None:
...
"""
        with tempfile.TemporaryDirectory() as temporary:
            config, config_path, project = create_project(Path(temporary), builder)
            report = lab_slides.audit_config(config, config_path, project)
        self.assertTrue(
            any("must not create a fresh presentation or add fresh slides" in item for item in report["errors"]),
            report["errors"],
        )

    def test_title_overflow_is_reported(self) -> None:
        builder = """
